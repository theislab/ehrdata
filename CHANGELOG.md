...
### Fixed
 - In {func}`~ehrdata.io.omop.setup_variables`, the units are now carried along the aggregation: from the very data point that is kept for `"last"`/`"first"`, and shared by the data points that are combined otherwise. Since combining values of different units is not meaningful, `aggregation_strategy` other than `"last"`, `"first"` and `"count"` now raises a `NotImplementedError` naming the features that have more than one `unit_concept_id`. ([#300](https://github.com/theislab/ehrdata/issues/300)) @eroell
