...
- `~ehrdata.io.write_zarr` now writes an `EHRData` specific store, with `AnnData` in a substore.
- `~ehrdata.io.write_zarr` now writes an `EHRData` specific store, with `AnnData` in a substore. This change allows to use `AnnData`s change to consolidated Zarr metadata. ([#185](https://github.com/theislab/ehrdata/pull/185)) @eroell
- `~ehrdata.io.read_zarr` is adapted to read the new store format, and can also deal with `AnnData` stores. ([#185](https://github.com/theislab/ehrdata/pull/185)) @eroell
